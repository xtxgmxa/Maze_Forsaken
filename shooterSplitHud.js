...
export function layoutShooterSplitHud(viewports, W, H, hudOpts = {}) {
  ...
  viewports.forEach((vp, i) => {
    ...
    const pad = 8;
    const radarSize = Math.min(84, Math.floor(Math.min(vp.w, vp.h) * 0.22));
    const barH = Math.min(44, Math.floor(vp.h * 0.1));
    ...
    if (s.top) {
      s.top.style.left = `${pad}px`;
      s.top.style.top = `${pad}px`;
      s.top.style.width = `${Math.max(120, vp.w - radarSize - pad * 3)}px`;
    }
    if (s.canvas) {
