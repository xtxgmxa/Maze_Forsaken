...
function getPlayerDisplayName(target) {
  return target?.displayName || target?.charDef?.name || "玩家";
}
...
function getLabelEl(key) {
  ...
  wrap.className = "shooter-lbl";
  wrap.dataset.key = key;
  const role = document.createElement("span");
  role.className = "shooter-lbl-role";
  const name = document.createElement("span");
  name.className = "shooter-lbl-name";
  wrap.append(role, name);
  layer.appendChild(wrap);
  labelPool.set(key, wrap);
  return wrap;
}
...
export function syncShooterOverheadLabels({
  ...
  for (const vp of viewports) {
    ...
    for (const target of players) {
      ...
      used.add(key);
      const wrap = getLabelEl(key);
      const roleEl = wrap.querySelector(".shooter-lbl-role");
      const nameEl = wrap.querySelector(".shooter-lbl-name");
      if (roleEl) {
        roleEl.className = `shooter-lbl-role ${rel.cls}`;
        roleEl.textContent = rel.text;
      }
      if (nameEl) {
        nameEl.textContent = getPlayerDisplayName(target);
      }
