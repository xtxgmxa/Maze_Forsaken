...
export function renderShooterScoreboard(players, human, playStyle = "teams") {
  ...
  tbody.innerHTML = rows.map((p, i) => {
    ...
    const name = p.displayName || p.charDef?.name || (p.isAI ? "Bot" : "Player");
    ...
    const moleTag = p.isMole && p._moleRevealed ? ' <span class="sb-mole">內鬼</span>' : "";

    return `<tr class="${you ? "you" : ""} ${dead ? "dead" : ""}" style="--row-team:${teamCss}">

      <td>${i + 1}</td>

      <td class="sb-name">${name}${p.isAI && !p.isLanPeer ? ' <span class="sb-bot">AI</span>' : ""}${moleTag}</td>
...
export function bindShooterScoreboardUi(onToggle) {
  ...
  if (bindShooterScoreboardUi._bound) return;

  bindShooterScoreboardUi._bound = true;
  ...
  if (sbBtn) {
    ...
    sbBtn.addEventListener("touchend", (ev) => {
